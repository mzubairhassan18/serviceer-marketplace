  ...
  return (
    <section className="category-section" aria-labelledby="category-heading">
      <div className="section-heading-row">
        ...
        <Link href="/#services">Browse all <span>→</span></Link>
      </div>
      <div className="category-grid">
        {categories.map(({ name, icon: Icon, note }, index) => (
          <Link key={name} href={index === categories.length - 1 ? "/#services" : `/?category=${name.toLowerCase()}#services`} className={`category-tile category-${index + 1}`}>
